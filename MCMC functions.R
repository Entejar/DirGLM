...
tht_sol <- function(spt, f0, mu, thtst){
  out <- gldrm:::getTheta(spt = spt, f0 = f0, mu = mu, sampprobs = NULL, 
                          ySptIndex = NULL, thetaStart = thtst)
  tht <- out$theta 
  bpr2 <- out$bPrime2
  btht <- apply(exp(outer(tht, spt, "*")), 1, function(row) log(sum(row * f0)))
...
#'
#' @examples
f0_update <- function(y, spt, cr_f0, cr_f0y, cr_dir_parm, cr_mu, cr_tht, cr_bpr2,
                      cr_btht, dir_pr_parm, ind_mt){
  n <- length(y)
  l <- length(spt)
  ...
  for(j in 1:l){
    ...
    if(sum(pr_f0 < 1e-3) == 0){
      ...
      pr_pf0 <- dbeta(pr_f0[j], dir_pr_parm[j], sum(dir_pr_parm[-j]), log = T)
      cr_pf0 <- dbeta(cr_f0[j], dir_pr_parm[j], sum(dir_pr_parm[-j]), log = T)
      cr_qf0 <- dbeta(cr_f0[j], pr_dir_parm[j], sum(pr_dir_parm[-j]), log = T)
      pr_qf0 <- dbeta(pr_f0[j], cr_dir_parm[j], sum(cr_dir_parm[-j]), log = T)
  ...
  return(list(cr_f0 = cr_f0, cr_f0y = cr_f0y, cr_tht = cr_tht, cr_btht = cr_btht,
              cr_bpr2 = cr_bpr2))
}
...
#'
#' @examples
beta_update_joint <- function(X, y, spt, cr_bt, cr_Sig, cr_f0, cr_tht,
                              cr_bpr2, cr_btht, rho){
  n <- dim(X)[1]
  l <- length(spt)
  
  pr_bt <- mvtnorm::rmvnorm(1, mean = cr_bt, sigma = cr_Sig) %>% as.vector()
  pr_mu <- exp(X %*% pr_bt) %>% as.numeric()
  ...
  if(sum(spt[1] <= pr_mu & pr_mu <= spt[l]) == n){
    ...
    cr_qbt <- dmvnorm(cr_bt, mean = pr_bt, sigma = pr_Sig, log = T)
    pr_qbt <- dmvnorm(pr_bt, mean = cr_bt, sigma = cr_Sig, log = T)
  ...
  return(list(cr_bt = cr_bt, cr_tht = cr_tht, cr_btht = cr_btht, cr_bpr2
              = cr_bpr2))
}
...
#'
#' @examples
beta_update_separate <- function(X, y, spt, cr_bt, cr_Sig, cr_f0, cr_tht,
                                 cr_bpr2, cr_btht, rho){
  n <- dim(X)[1]
  p <- dim(X)[2]
  l <- length(spt)
  ...
  for(j in 1:p){
    ...
    if(sum(spt[1] <= pr_mu & pr_mu <= spt[l]) == n){
      ...
      cr_qbt <- dnorm(cr_bt[j], mean = pr_bt[j], sd = pr_sd[j], log = T)  
      pr_qbt <- dnorm(pr_bt[j], mean = cr_bt[j], sd = cr_sd[j], log = T)
